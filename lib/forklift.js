...
class Forklift {
    ...
    constructor(options) {
        ...
        const {error, value: optionsWithDefault} = Joi.validate(options, constructorOptionsValidation);
        if (error) {
            throw error;
        }
    ...
    async upload({source, remotePath, options = {}}) {
        ...
        const {error, value: optionsWithDefault} = Joi.validate(options, uploadOptionsValidation);
        if (error) {
            throw error;
        }
